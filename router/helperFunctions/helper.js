...
const getCart = async (userId) => {
  try {
    ...
    const productIds = cart[0]?.products?.map((item) => item.product_id);

    // Fetch products based on product IDs
    const products = await Product.find({ _id: { $in: productIds } });

    const cartWithProductDetails = cart[0]?.products?.map((cartItem) => {
      ...
      return {
        ...
        price: productDetail?.product_price, // Nest product details inside the cart item
        discount: productDetail?.product_discount, // Nest product details inside the cart item
        color: productDetail?.product_color, // Nest product details inside the cart item
        user_id:userId
      };
    });
...
const SendMailFunction=async (sendto,userLogin,res)=>{
  ...
  const link = `https://www.siddhisarees.com/forgetpass/?token=${token}&user_id=${userLogin?._id}`;
  console.log(link)
  const htmlContent = `
    <div>
      <p>Forgot Pass</p>
